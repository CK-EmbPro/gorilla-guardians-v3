                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      {conv.unreadCount > 0 && (
                        <Badge className="bg-primary text-primary-foreground text-xs h-5 min-w-5 px-1 flex items-center justify-center" data-testid={`badge-unread-${conv.id}`}>{conv.unreadCount}</Badge>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground line-clamp-1">{conv.lastMessage ?? "Start a conversation"}</p>
                    <p className="text-[10px] text-muted-foreground/50 mt-0.5 capitalize">{other?.role}</p>
                  </button>
                );
              })
            )}
