...
import { Calendar, Clock, Users, DollarSign, ChevronDown, ChevronUp, RefreshCw, XCircle } from "lucide-react";
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-3 gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Booking #{b.id}</p>
          </div>
          ...
          <div className="flex gap-2">
            ...
            {b.experience?.slug && (
              <Link href={`/experiences/${b.experience.slug ?? b.experience.id}`}>
                <button className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md border border-primary/30 text-primary hover:bg-primary/5 transition-colors font-medium">
...
export default function CustomerBookingsPage() {
  ...
  const { data: bookings, isLoading } = useListBookings({ userId: user?.id, limit: 50 });
