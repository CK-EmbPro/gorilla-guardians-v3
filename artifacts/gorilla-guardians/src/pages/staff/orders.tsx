                  ...
                  <tbody>
                    ...
                    {filtered.map((o: any) => (
                      <tr key={o.id} className="border-b border-border hover:bg-muted/20 transition-colors" data-testid={`row-staff-order-${o.id}`}>
                        ...
                        <td className="px-4 py-3 text-muted-foreground text-xs">{o.items?.map((i: any) => i.product?.name ?? "Item").join(", ") ?? "—"}</td>
      ...
      <Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
        <DialogContent>
          ...
          <div className="py-4 space-y-4">
            <p className="text-sm text-muted-foreground">Order <span className="font-medium text-foreground">ORD-{String(selectedOrder?.id).padStart(3,"0")}</span></p>
          ...
          <DialogFooter>
            ...
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleUpdateStatus} disabled={updateOrder.isPending}>
              {updateOrder.isPending ? "Saving..." : "Update Status"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
